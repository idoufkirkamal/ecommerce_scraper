...
# Load the data
df = pd.read_csv(
    r"C:\Users\AdMin\Desktop\ecommerce_scraper\data\raw\ubuy\graphics_cards\graphics_cards_2025_01_30_scrape1.csv")
...
def clean_memory_size(memory_ram, graphics_card_ram_size, title):
    def sanitize(value):
        if pd.notna(value):
            # Remove non-numeric and special characters
    ...
    try:
        # Prioritize `memory_ram`
        if memory_ram:
            return int(memory_ram)
        # Fallback to `graphics_card_ram_size`
        elif graphics_card_ram_size:
            return int(graphics_card_ram_size)
        else:
            # Attempt to extract from title if no RAM information is directly available
            match = re.search(r'(\d+)\s*GB', title)
            return int(match.group(1)) if match else None
    except ValueError:
        # If conversion fails for any unexpected reason
        return None


df['Memory Size'] = df.apply(lambda row: clean_memory_size(row['RAM'], row['Graphics Card Ram Size'], row['title']),
                             axis=1)
...
df['Memory Type'] = df.apply(lambda row: clean_memory_type(row['Memory Speed'], row['Graphics RAM Type'], row['title']),
                             axis=1)


# Clean Chipset/GPU Model
...
df['Chipset/GPU Model'] = df.apply(
    lambda row: clean_chipset_gpu_model(row['Graphics Processor Manufacturer'], row['GPU Clock Speed'], row['title']),
    axis=1)
...
# Replace the deprecated fillna method with the recommended ffill() and bfill() methods
df.ffill(inplace=True)  # Forward fill
df.bfill(inplace=True)  # Backward fill


# Alternatively, use a dictionary to map common brands/models to their typical specs
...
# Save cleaned data
df_cleaned.to_csv('cleaned_graphics_cards.csv', index=False)